...
const { data: dashboardData, pending } = await useAsyncData('admin-stats', () => adminRepo.getDashboardStats(token.value!));
...
const { data: logsData } = await useAsyncData('admin-logs', () => adminRepo.getAuditLogs(token.value!));

const recentLogs = computed(() => logsData.value?.slice(0, 5) || []);
</script>
